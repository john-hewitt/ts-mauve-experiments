...
def my_top_k_top_p_filtering(
    ...
    if eta > 0.0:
        ...
        cutoff = torch.min(torch.sqrt(torch.tensor(eta).to(entropy.device))*torch.exp(-entropy), torch.tensor(eta).to(entropy.device))
        indices_to_remove = probabilities < cutoff.reshape(-1).unsqueeze(1).expand(-1,probabilities.size()[-1])
        max_word = torch.argmax(logits, dim=-1)
        indices_to_remove[...,max_word.squeeze()] = 0
        logits[...,indices_to_remove] = filter_value
	...
	# calculate entropy
        ...
        last_ind = (cumulative_probs < typ).sum(dim=1)
...
@torch.no_grad()
def featurize_sequential(model, ds_tokens, min_len=0):
    device = next(model.parameters()).device
    t1 = time.time()
    feats = []
    for sen in tqdm(ds_tokens):
        print(sen.size()[-1])
        if sen.size()[-1] < min_len:
            print('omit', sen.size()[-1])
            continue
        sen = sen.to(device)
        outs = model(input_ids=sen, past_key_values=None,
             output_hidden_states=True, return_dict=True)
